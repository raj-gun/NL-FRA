function [Norm_SSE_abs_LS_2,Norm_SSE_arg_LS_2,Norm_SSE_LS_2,Fe_n_Yn,Fe_p_Yn,Y_NOFRF_MLS_n,Y_NOFRF_LS_2,G_LS_2,sse,Y_model,rmv_U_mat] = ...
    narx_nofrf_comp(Fs,Ts,tspan,fftn,pos_freq_comp,u_nofrf,A,Amp,zero_cross,nl_ord_set,gc,harm_inpt,lw,norm,displ,model)
%------------
...
% y_lag_lin_srt=model{13};

min_dyn_ord_y=model{1};
max_dyn_ord_y=model{2};
min_dyn_ord_u=model{3};
max_dyn_ord_u=model{4};
theta_procss=model{5};
...
bias=model{11};
inpt0=model{12};
y_lag_lin_srt=model{13}; y_lag_lin_srt=y_lag_lin_srt.*0;
%------------
u_org=u_nofrf;
len = length(u_nofrf);
n_A = length(A);
...
[Norm_SSE_abs_LS_2,Norm_SSE_arg_LS_2,Norm_SSE_LS_2,Fe_n_Yn,Fe_p_Yn,Y_NOFRF_MLS_n,Y_NOFRF_LS_2,Y_model] = ...
    NOFRF_Y_pred_comp(pos_freq_comp, u_nofrf, y_test, Ts, Fs, N, Amp, len_adj, len_adj_hlf, G_LS_2, rmv_U_mat, freq_rng_all, gc, harm_inpt, lw, norm, displ(2));
