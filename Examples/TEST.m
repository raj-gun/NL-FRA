clear all;clc;close all

addpath('C:\Users\rajin\OneDrive - Coventry University\PhD project\GitHub\NL-FRA\NOFRF\');
%%
Fs = 80000;
Ts = 1/Fs;%(2*pi)
tspan = [-5+0.5*Ts:Ts:5+0.5*Ts]; %tspan_org = tspan;
fftn = length(tspan)-1;
%% Generate data using an ODE and RK4 to evaluate NOFRFs

f1=70; f2=50; %f1 > f2 always!
Amp_2 = 9.8;
% u = Amp_2.*( 0.6 + 0.1*cos(2.*f1.*pi.*tspan)); 
u = Amp_2 .* (3/(2*pi)).*(sin(2.*f1.*pi.*tspan)-sin(2.*f2.*pi.*tspan))./tspan;

len = length(u);

%-------------------------------
A = [1, 0.85, 0.7, 0.55, 0.4]';% [linspace(1.1,1.25,5)']; %
n_A = length(A);
Y = zeros(len,n_A);

ode_func = @(t,y,Amp) ODE_func(t,y,f1,f2,Amp,Amp_2); %ODE function
y0 = [0,0]';
Y = NOFRF_data_sim(ode_func,tspan,n_A,A,Y,y0);%Simulate system to obtain dataset for NOFRFs

%-------------------------------

%% NOFRFs testing

%--------------------------
Amp = 1;
%tspan = tspan_org;

% u = Amp_2.*( 0.6 + 0.1*cos(2.*f1.*pi.*tspan)); 
% u = Amp_2 .* (3/(2*pi)).*(sin(2.*f1.*pi.*tspan)-sin(2.*f2.*pi.*tspan))./tspan;

y0 = [0 0]';
y = ode4(@(t,y) ode_func(t,y,Amp),tspan,y0); % RK4-based simulation
y_test = y(:,1);
%% Downsample to 1000 Hz

M = 20;
[u,      Fs_new] = FIR_downsample(u', Fs, M);
tspan = tspan(1:M:end);
[Y,      Fs_new] = FIR_downsample(Y, Fs, M);
[y_test, Fs_new] = FIR_downsample(y_test, Fs, M);
Fs = Fs_new;
Ts = 1/Fs;%(2*pi)
fftn = length(tspan)-1;

% [c,zc_tspan] = min(abs(tspan));
% tspan = tspan(zc_tspan:end);
% u = u(zc_tspan:end);
% Y = Y(zc_tspan:end,:);

len = length(u);
len_adj = len; len_adj = fftn;
len_adj_hlf = floor(len_adj/2)+1;
w = [0:Fs/len_adj:(Fs/2)];%[0:Fs/len_adj:(Fs/2)+(Fs/len_adj)];%


disp(['Inpt Amps = ',num2str(A')]);
disp('Simulations complete');

u_fft = (Ts/len).*fft(u,len_adj);
figure;plot(w, abs(u_fft(1:len_adj_hlf)));

figure;plot(tspan, u);



%% Evaluate NOFRFs

nl_ord_set = [1:5];
N = max(nl_ord_set);
displ = 1;

gc= 'b';
harm_inpt = 0;
lw=0.5;
displ = [1,1,1];
norm = 0;

u_nofrf = u;

[Norm_SSE_abs_LS_2,Norm_SSE_arg_LS_2,Norm_SSE_LS_2,Fe_n_Yn,Fe_p_Yn,Y_NOFRF_MLS_n,Y_NOFRF_LS_2,G_LS_2,sse,Y_model] = ...
    SISO_NOFRF(Fs,Ts,tspan,fftn,f1,f2,u_nofrf,A,Amp,nl_ord_set,gc,harm_inpt,lw,norm,displ,Y, y_test);

%%
%%
%% Local Functions
%%
%%
%% ODE func

% function dy=ODE_func(t,Y,f1,f2,Amp)
% %%
% PE = Amp.*(3/(2*pi)).*(sin(2.*f1.*pi.*t)-sin(2.*f2.*pi.*t))./t;%Amp.*( 0.6 + 0.1*cos(2.*f1.*pi.*t));% + Amp.*(sin(2.*f2.*pi.*t));% + Amp.*(sin(2.*3*f1.*pi.*t)); Amp.*cos(2.*f1.*pi.*t);%
% PI = 0;%Amp.*( 0.6 + 0.1*cos(2.*f1.*pi.*t));
% %%
% tau_e = 0.0035; tau_i = 0.0052; 
% T_EE = 2.4; T_EI = 2; T_IE = 2.1; T_II = 0;
% r =4; S_0 = 1; v_0 = 1;
% 
% 
% x1 = PE + T_EE*Y(1) - T_IE*Y(2);
% x2 = PI + T_EE*Y(1) - T_II*Y(2);
% 
% dy = [(-1/tau_e)*Y(1) + (1/tau_e)*( S_0 / ( 1 + exp(-r*(x1-v_0)) ) );...
%       (-1/tau_i)*Y(2) + (1/tau_i)*( S_0 / ( 1 + exp(-r*(x2-v_0)) ) )];
% 
% 
% end


function dy=ODE_func(t,Y,f1,f2,Amp,Amp_2)

u = Amp_2 .* (3/(2*pi)).*(sin(2.*f1.*pi.*t)-sin(2.*f2.*pi.*t))./t;
u = Amp.*u;

M = 0.013;
C = 0.0607;
K1 = 40;
K3 = -2.1e5;
K5 = 4.21e9;

dy = [Y(2);...
      (1/M)*(u - C*Y(2) - K1*Y(1) - K3*Y(1)^3 - K5*Y(1)^5)];

end

function [y,Fs_new] = FIR_downsample(x,Fs,M)

% New sampling frequency
Fs_new = Fs/M;

% Low-pass filter frequencies
Fstop = Fs_new/2;       % New Nyquist frequency
Fpass = 0.8*Fstop;      % Passband edge

% Filter specifications
Rp = 0.01;              % Passband deviation
Rs = 60;                % Stopband attenuation (dB)

dev = [Rp 10^(-Rs/20)];

% Automatically determine FIR filter order
[N,Wn,beta,ftype] = kaiserord([Fpass Fstop], ...
                              [1 0],dev,Fs);

% Design FIR filter
b = fir1(N,Wn,ftype,kaiser(N+1,beta),'noscale');

% Apply low-pass filter
x_filt = filtfilt(b,1,x);

% Downsample
y = x_filt(1:M:end,:);

end