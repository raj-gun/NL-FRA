function [G_LS_2,Y_LS_2,U_mat,Y_mat,rmv_U_mat,freq_rng_nonlinear_ord,len_adj_hlf,sse] = NOFRF_LS2comp(u,Y,A,N,Fs,Ts,pos_freq_comp,nl_ord_set,fftn,displ)
...
[freq_rng_nonlinear_ord,freq_rng_nonlinear_ord_NOFRF] = nonlinear_freq_comp(N,pos_freq_comp,len_adj,Fs);
...
error = Y_mat(freq_rng_all,:) - Y_LS_2(freq_rng_all,:);
sse_norm = sum(abs(error).^2,1) ./ var(abs(Y_mat(freq_rng_all,:)));

sse_norm_real = sum(real(error).^2,1) ./ var(real(Y_mat(freq_rng_all,:)));
sse_norm_imag = sum(imag(error).^2,1) ./ var(imag(Y_mat(freq_rng_all,:)));
